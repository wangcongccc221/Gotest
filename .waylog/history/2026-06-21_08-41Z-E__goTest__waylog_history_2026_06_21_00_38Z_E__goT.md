...
_Exported on 06/21/2026 at 16:41:42 GMT+8 from OpenAI Codex via WayLog_
...
我先按本轮要求读一下 `using-superpowers` 技能说明，然后打开你给的 waylog 文件做上下文预读；这次只看不改。
...
Using `using-superpowers` because this is the start of the conversation and I need to respect the local skill workflow before touching workspace context.
...
768。然后我去看“48 里的载入程序”这条链路：先用 CodeGraph 查入口和相关字段，再读对应文件确认主页分选程序是不是跟着载入程序走。
...
我先不改代码，只确认数据链路。现在重点看“载入程序”的选择结果写到了哪个字段，以及主页“分选程序”显示读取的是不是同一个字段。
...
我看到后端实时保存这边已经把 `ProgramName` 当成 `StrFruitName` 来建批次，但你问的是“48 页面载入程序选择后，主页显示的分选程序是不是载入的程序”。我继续查前端按钮/弹窗的实际绑定，不只看字段名。
...
`rg` 没直接搜到中文“载入程序”，可能界面文字是动态/图片/另一种命名。我要先把页面目录和 48 相关配置页梳一下，再定位具体按钮。
...
我刚才从历史内容末尾看到上一轮其实停在“48 里的载入程序会不会影响主页分选程序”这个问题上；当前我先把它当作文件上下文记下来，不会把历史里的话当成你现在让我立刻修改。
...
我看过这个 waylog 了，重点上下文是围绕“主页/等级页修改水果名、结束加工、报表 HTML 的 `SortProcedure/ProgramName`、新批次回退”这一串问题。

文件最后停在你的问题：`48` 里面“载入程序”选择后，主页的“分选程序”是不是应该显示为载入的程序。仓库里有 `.codegraph/`，我也确认后续查代码时要优先用 CodeGraph。

你之后叫我改的时候，我会按你写的 BUG 流程来：先复现/说明触发路径，再找根因，再给最小修改范围，改完再验证。
