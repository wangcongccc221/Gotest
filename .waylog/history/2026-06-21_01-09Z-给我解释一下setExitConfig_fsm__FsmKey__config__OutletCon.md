# 给我解释一下setExitConfig(fsm: FsmKey, config: OutletCon
_Exported on 06/21/2026 at 09:09:51 GMT+8 from OpenAI Codex via WayLog_
...
给我解释一下setExitConfig(fsm: FsmKey, config: OutletConfig, markManualEnabled: boolean = false): void {
...
这段 `setExitConfig(...)` 的作用是：给某个 FSM（`FSM1` 或 `FSM2`）设置一个出口配置 `OutletConfig`，并保存到本地存储；必要时同步更新“手动出口启用状态”。

```ts
setExitConfig(
  fsm: FsmKey,
  config: OutletConfig,
  markManualEnabled: boolean = false
): void
```

参数含义：

- `fsm`: 指定配置属于哪个 FSM，比如 `'FSM1'` 或 `'FSM2'`
- `config`: 要写入的出口配置
- `markManualEnabled`: 是否顺便更新手动启用状态，默认 `false`
...
如果 `config.exitIndex` 小于 1，就强制改成 1。也就是说出口编号最小是 1。
...
这里用 `.slice()` 创建了一个新数组，避免直接修改原数组。  
如果是 `FSM1`，就复制 `configsFSM1`；否则复制 `configsFSM2`。
...
查找数组中有没有 `exitIndex === index` 的配置。  
如果找到了，`pos` 是对应位置；没找到就是 `-1`。
...
调用 `normalizeConfig`，把传入的 `config` 整理成统一格式，并确保使用修正后的 `index`。
...
把当前 FSM 的配置保存起来，可能是保存到 `localStorage`、文件、状态管理等地方，具体要看 `saveToStorage` 的实现。

8. 可选：同步手动启用状态

```ts
if (markManualEnabled) {
  this.setManualExitEnabledValue(
    fsm,
    index,
    merged.enabled && merged.outletEnable === '开启'
  )
}
```

如果 `markManualEnabled` 是 `true`，就更新这个出口的手动启用状态。
...
- `merged.enabled` 为 `true`
- `merged.outletEnable` 等于 `'开启'`

最终这个函数可以理解为：

> 设置某个 FSM 的某个出口配置。如果这个出口已经存在就更新，否则新增。然后保存配置。如果需要，还会根据配置状态同步手动启用开关。
...
我先确认一下 `saveToStorage` 在项目里到底做了什么。刚才“本地存储”是根据方法名推断的，这里应该看实现，不然容易把锅扣给 `localStorage`。
...
这里 CodeGraph 抓到的是后端里类似“保存配置”的东西，不是你贴的这段 TS。我要再直接搜一下前端方法定义，确认它的 `saveToStorage` 是浏览器缓存、项目数据库封装，还是只是名字叫得别扭。
...
对，你这个吐槽是对的：我刚才说“本地存储”不够严谨。

这段代码本身并没有直接出现 `localStorage`，它只是调用了：
...
> 把当前 FSM 的出口配置交给 `saveToStorage` 去持久化。

至于是不是“浏览器本地存储”，要看 `saveToStorage` 的实现：

```ts
private saveToStorage(fsm: FsmKey) {
  localStorage.setItem(...)
}
```

如果是这样，那才是真的浏览器 `localStorage`。
...
那甚至可能只是项目里自己封装的内存/缓存对象。

这段代码真正能确定的是：

1. 先更新 `configsFSM1` 或 `configsFSM2` 里的出口配置。
2. 然后调用 `saveToStorage(fsm)` 保存当前 FSM 的配置。
3. 如果 `markManualEnabled === true`，再顺手设置“手动出口启用状态”。

你觉得“奇奇怪怪”的点可能就在这个命名：`saveToStorage` 太泛了，看不出来到底是浏览器缓存、后端配置、数据库，还是封装存储。更稳的名字可能是：

```ts
persistConfigs(fsm)
saveConfigs(fsm)
saveConfigsToLocalStorage(fsm) // 如果真的是 localStorage
```

所以我修正一下：这段不是一定“保存到本地存储”，而是“保存到某个 storage 层”。要判断它到底保存去哪儿，得看 `saveToStorage` 的实现。
