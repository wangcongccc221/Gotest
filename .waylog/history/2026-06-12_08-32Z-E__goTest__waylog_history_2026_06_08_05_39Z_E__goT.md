...
_Exported on 06/12/2026 at 16:33:21 GMT+8 from OpenAI Codex via WayLog_
...
Using `superpowers:using-superpowers` as required at conversation start; for this request I’m just reading the referenced note and summarizing what’s in it.
...
看了这份 waylog，当前可接上的重点是：

1. 前面刚修过 **重量波形图**：
   - 改过 `WaveCaptureDialog.ets`
   - 改过 `WeightSettingsPage.ets`
   - 新增过 `verify_wave_capture_plot.mjs`
   - 按要求没有编译，只做了源码检查脚本和 `git diff --check`

2. 最后一个明确任务停在这里：  
   你让对比 **鸿蒙和 48 的主页、等级界面差距**。

3. 当时结论是：
   - 主页主要差距：还保留 FSM1/FSM2、出口菜单少复制/剪切/粘贴、底部状态按钮部分只是日志、出口卡片单击打开设置、视觉密度比 48 松。
   - 等级界面主要差距：取消不是真取消、任意等级内容变化会清空出口卡片、保存前校验不足、下发失败无回滚、没按硬件能力限制、减少等级数量缺加工中确认、缺少按行插入/删除。

4. 建议下一步优先级是：
   - 先修等级界面：取消、保存校验、只在等级数量变化时清空出口映射。
   - 再处理主页 FSM 和出口菜单。

我已经接上这个历史上下文了。当前还没有改代码，也没有编译。
...
其实我想要修改一下这个点开这个出口设置里面的等级分配的这个表 你先看一下
...
我先只看出口设置弹窗里“等级分配”这张表的代码路径和数据流，不改文件、不编译。重点会确认它从哪打开、表格数据从哪里来、保存时写回哪里。
...
刚才按 IDE 里的 `my_harmony/...` 搜不到，是因为当前工作区根目录本身就是项目根，代码实际在 `entry/src/main/ets`。我继续沿着出口卡片点击和设置弹窗往里追。
...
Using `superpowers:brainstorming` because this is heading toward a UI/behavior change, but I’ll keep this pass to code-reading only as requested.
...
当前 `E:\goTest` 这份工程不是 IDE 打开的那套完整鸿蒙代码，只剩一个很小的 `entry` 骨架；历史里真正带出口卡片和设置弹窗的是 `E:\new\my_harmony`。我现在只读那边相关文件，把“点开出口设置 -> 等级分配表”的链路摸清。
...
我已定位到候选文件，出口设置看起来在 `pages/home/OutletDialog.ets`，不是通用 `components/dialogs` 目录。接下来我看它的构造参数、表格渲染和保存回调，顺便追一下从卡片点击传进来的出口编号。
...
表格本体已找到：`CompositeGradeTable` 被 `OutletDialog` 的 `buildGradePanel()` 嵌进去。现在我再读表格组件和 `GradeInfoConfigManager` 的读写函数，确认你后面要改 UI 还是改勾选/保存语义时会碰到哪些边界。
...
表格当前支持“单元格、行头、列头、全部等级”点击选择，并且弹窗确定时一次性写回该出口的映射。最后我查一下打开弹窗和确定保存的上层逻辑，确认是否会即时下发设备。
...
看完了。出口设置里的“等级分配”表不是在 `WeightSettingsPage.ets`，实际链路在这几个文件：

- [OutletDialog.ets](E:/new/my_harmony/entry/src/main/ets/pages/home/OutletDialog.ets:674)：弹窗右侧“等级分配”区域
- [CompositeGradeTable.ets](E:/new/my_harmony/entry/src/main/ets/components/tables/CompositeGradeTable.ets:1)：表格本体
- [Home.ets](E:/new/my_harmony/entry/src/main/ets/pages/Home.ets:4481)：弹窗打开和确定保存
- [GradeInfoConfigManager.ets](E:/new/my_harmony/entry/src/main/ets/pages/home/core/GradeInfoConfigManager.ets:376)：读取/写回出口等级映射

当前数据流是：

`点击出口卡片` → `Home.openOutletDialogForIndex()` 准备等级表数据和出口配置 → `OutletDialog.syncFromConfig()` 读取当前出口已分配等级 → `CompositeGradeTable` 显示表格 → 点击单元格/行头/列头修改选择 → 点“确定” → `Home.onConfirm()` 调 `setExitMappingsFromGradeCells()` 或 `setExitMappingsFromGradeNames()` → 更新 `StGradeInfo.grades[].exit` 位掩码 → 刷新出口卡片并下发设备。

这个表现在支持：
- 点单个格子选择/取消等级
- 点行头整行选择/取消
- 点列头整列选择/取消
- 只有尺寸等级时点左上角“等级”全选/取消
- 横向滚动
- 在出口设置里禁用了拖拽，只保留点击选择

我也注意到一个可能是你想改的点：这个表是复用主页的综合等级表，所以视觉和交互比较“表格化”，不像 48 那种出口设置里的紧凑勾选分配区。如果你要改它，主要会动 `OutletDialog.buildGradePanel()` 和 `CompositeGradeTable.ets`，保存逻辑暂时不用动。当前我没改代码，也没编译。
...
我点击这个列 或者头是进行多选操作是吧 我多选确定 可以进行添加到出口的操作 但是我取消勾选（多选） 就没有办法删除 我进行单选的话 就可以进行删除 你看一下是什么情况
...
Using `superpowers:systematic-debugging` and `superpowers:test-driven-development` for this bug: first reproduce the logic path, then lock the failing multi-uncheck case with a focused source-level check before the smallest fix.
...
我先把这个当真实 bug 追，不猜。现象已经能缩小到“行/列头批量取消后，弹窗状态传给保存层的数据没有表达删除，或保存层没有按批量状态覆盖出口映射”这两段之间。
...
关键代码现在已经圈住：单选和行/列头多选都走同一个 `toggleSelectedGrade()`，但多选取消是一轮循环里连续调用很多次单项删除。我要验证的是这批连续删除在状态数组里是不是被“旧状态覆盖”，这是最符合“单选能删、多选删不掉”的形态。
...
根因假设我现在能明确写出来：行/列头取消不是一次“批量设置”，而是短时间内连续触发多个单格取消；`@State` 数组在这种连续回调里可能按旧快照更新，最后保存层还优先用残留的 `selectedGradeCells`，所以视觉上像取消了，确定时又把旧格子写回。单格取消只有一次回调，所以不会撞到这个问题。
...
修复方案已经定了，范围很小：不改保存协议、不改 `StGradeInfo` 位掩码，只给出口弹窗维护一份同步的等级选择草稿数组。这样行/列头一次触发多项取消时，后续每一次取消都基于上一项取消后的结果，而不是基于可能还没刷新的 `@State`。
