...
_Exported on 06/15/2026 at 14:04:08 GMT+8 from OpenAI Codex via WayLog_
...
- browser:control-in-app-browser: Control the in-app Browser. Use to open, navigate, inspect, test, click, type, screenshot, or verify local targets such as localhost, 127.0.0.1, ::1, file://, the current in-app browser tab, and websites shown side by side inside Codex. (file: r3/browser/26.609.41114/skills/control-in-app-browser/SKILL.md)
...
我先按项目规则把这个文件读出来，确认你指向的内容和上下文；不编译、不改代码。
...
我读到这是一个 WayLog 导出的历史会话，里面后半段在讨论“加工统计/品质选项/硬编码”的问题。我再精准拉一下“加工曲线”和“品质1”附近上下文，避免把整份 1300 行记录读歪。
...
使用 `superpowers:systematic-debugging` 和 `superpowers:test-driven-development`：这是一个疑似硬编码导致的 UI 数据源 bug，要先锁根因和回归条件，再做最小改动。
...
我开始查实际工程里的下拉框来源：先找 `品质1` 和“品质选项”字符串，再沿数据源往上追，确认它是占位默认值、配置兜底，还是直接写死。
...
初步状态：当前仓库有上一轮图表留白改动尚未提交，我会保留它，不回退。现在继续定位“品质1”具体来自哪个组件。
...
已定位到 `CompositeGradeTable` 这条链，`品质1` 很可能不是下拉框固定数组，而是综合等级表在没有真实品质名时由 fallback 生成。下一步查 helper 里 fallback 名称规则，以及调用方为什么没拿到真实品质行。
...
有两个证据点：`GlobalDataInterface` 里确实有 `品质${index+1}` 兜底；而首页等级表的 `DataTablesTabBar` 又从表格第一列取品质名。如果真实配置没传进去，就会暴露这个兜底名。继续追这个 helper 和运行时配置生成处。
...
现在根因假设变成：`品质1` 是兜底标签，不是业务配置名；它应该只在完全无配置时内部兜底，不能出现在用户可选的“品质选项”下拉里。继续定位这个下拉框本体。
...
下拉框本体已确认在 `WeightSizeStatisticsBarChart.buildQualitySelector()`，实时模式选项来自 `RealtimeStatsService.getWeightSizeQualityOptions()`；所以修复点应该在统计服务的数据源过滤，而不是 UI 里把 `品质1` 文字替换掉。
...
根因已经收敛：实时下拉框调用 `resolveQualityNames()`，这个函数为了图表内部可显示，会在配置名缺失时生成 `品质${n}`；这对内部坐标标签能兜底，但不该作为用户下拉选项暴露。现在按 TDD 先加一个静态回归，要求下拉选项只来自真实配置名。
...
历史统计这条链本身只从数据库 `QualityName` 去重，不会主动生成 `品质1`；问题集中在实时统计服务。现在加回归脚本验证：实时下拉选项不能直接复用带 fallback 的 `resolveQualityNames()`。
...
我现在先落红灯脚本：它会要求 `getWeightSizeQualityOptions()` 不再把内部 fallback 名称当成下拉来源，而必须走一个只返回真实配置名的 helper。
