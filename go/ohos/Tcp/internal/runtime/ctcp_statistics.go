...
type cTCPStStatisticsSnapshotEntry struct {
	Stats    StStatistics
	LatestAt time.Time
	Stale    bool
}
...
func cacheStStatisticsForSpeed(state StStatistics, receivedAt time.Time) {
	...
	cTCPStStatisticsSpeedMu.Unlock()
	recordFSMStatsReceiveCSV(state, receivedAt)
	maybeResumeRealtimeSaveAfterStatisticsReset()
	maybeSaveRealtimeStatistics(receivedAt)
}

func latestStStatisticsSpeedSnapshotEntries(now time.Time) []cTCPStStatisticsSnapshotEntry {
	cTCPStStatisticsSpeedMu.Lock()
	defer cTCPStStatisticsSpeedMu.Unlock()

	entries := make([]cTCPStStatisticsSnapshotEntry, 0, len(cTCPStStatisticsSpeedBySys))
	for _, entry := range cTCPStStatisticsSpeedBySys {
		if entry == nil || !entry.HasLatest {
			continue
		}
		latest := entry.Latest
		stale := now.Sub(entry.LatestAt) > cTCPStStatisticsSpeedStaleInterval
		if stale {
			latest.NIntervalSumperminute = 0
		}
		entries = append(entries, cTCPStStatisticsSnapshotEntry{
			Stats:    latest,
...
func latestStStatisticsSpeedSnapshots(now time.Time) []StStatistics {
	entries := latestStStatisticsSpeedSnapshotEntries(now)
	snapshots := make([]StStatistics, 0, len(entries))
	for _, entry := range entries {
		snapshots = append(snapshots, entry.Stats)
	}
	return snapshots
}
...
func publishLatestStStatisticsSpeed(now time.Time) {
	entries := latestStStatisticsSpeedSnapshotEntries(now)
	recordFSMStatsSnapshotCSV(now, entries)
	snapshots := make([]StStatistics, 0, len(entries))
	for _, entry := range entries {
		snapshots = append(snapshots, entry.Stats)
	}
