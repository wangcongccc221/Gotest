...
3. 区分 WebSocket JSON、CTCP SYNC 二进制帧和 ArkTS 数据映射，并知道 `topic` 只是消息中的数据类别字段。
...
10. **推回前端**：Go 统一封装 WebSocket 数据帧并广播给全部连接，`topic` 只标识数据类别。
11. **映射与显示**：前端按 `topic` 字段分流，映射 ArkTS 结构体，写入状态并刷新 UI。
...
- 不把 `topic` 字段描述为 WebSocket 主题订阅；代码中没有订阅表，也没有 subscribe/unsubscribe 消息。
- Go 函数名中的 `Publish` 按实际行为讲成“封装后广播”，不引申为发布订阅架构。
